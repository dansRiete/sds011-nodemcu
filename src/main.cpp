...
#include <../lib/Adafruit_BME280_Library-master/Adafruit_BME280.cpp>

#define WIFI_MAX_RETRIES 20
#define MQTT_MAX_RETRIES 1
#define DEFAULT_MEASURING_DURATION 60*1000
...
#define MQTT_SUBSCRIBER "ESP8266-SmartHouse2"
...
void loop() {
    ...
    if (millis() - currentTimeMillisTimer > measuringDuration) {
        ...
        Measure currentMeasure = {
                ...
                NULL_MEASURE_VALUE,
                NULL_MEASURE_VALUE,
                static_cast<short>(round(temp * 100)),
                static_cast<short>(round(temp * 100)),
                static_cast<short>(round(temp * 100)),
                static_cast<short>(round(humid * 100)),
                NULL_MEASURE_VALUE,
                NULL_MEASURE_VALUE,
                true
        };
        ...
        static char measureString[MAX_MEASURES_STRING_LENGTH];
        snprintf(measureString, MAX_MEASURES_STRING_LENGTH,
                 "%s,balcT=%.1f,balcRh=%.0f,balcAh=%.1f",
                 getTimeString(currentMeasure.measureTime),
                 round2(temp),
                 round2(humid),
                 round2(calculateAbsoluteHumidity(temp, humid))
        );
